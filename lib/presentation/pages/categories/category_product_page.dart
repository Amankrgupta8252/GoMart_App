...
class CategoryProductPage extends StatelessWidget {
  ...
  CategoryProductPage({super.key, required this.title, required this.categoryKey}) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(onPressed: (){}, icon: Icon(IconsaxPlusBroken.search_favorite)),
          SizedBox(width: 8,),
        ],
      ),
      body: Obx(() {
  ...
  Widget _buildProductCard(Map<String, dynamic> item, String tag) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Text(item['title'] ?? 'No Title', style: const TextStyle(fontWeight: FontWeight.bold), maxLines: 1),
      Row(children: [
        const Icon(Icons.star, color: Colors.amber, size: 14),
        Text(" ${item['rate'] ?? '0'} | ${item['sold'] ?? '0'} sold", style: const TextStyle(fontSize: 12)),
      ]),
      Text("\$${item['price'] ?? '0'}", style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: Colors.black)),
    ]);
  }
}